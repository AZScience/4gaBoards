          ...
          {canAdd && (
            <div className={s.headerButton}>
              <ProjectAddPopup defaultData={defaultData} isSubmitting={isSubmitting} onCreate={onCreate} offset={16} position="bottom">
                <Button variant={ButtonVariant.NoBackground} title={t('common.addProject')} className={s.addButton}>
            ...
            <Link to={Paths.PROJECTS.replace(':id', item.id)}>
              <div className={s.project}>
                ...
                <div className={s.projectDetails}>
                  <div className={s.projectDetailText}>
                    {t('common.createdBy')}: {item.createdBy?.name || '-'}
                  </div>
                  <div className={s.projectDetailText}>
                    {t('common.createdAt')}: {item.createdAt ? t('format:date', { postProcess: 'formatDate', value: item.createdAt }) : '-'}
                  </div>
                </div>
              </div>
            </Link>
